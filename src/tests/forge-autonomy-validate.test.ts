...
describe("forge.autonomy.validate", () => {
  ...
  test("passes when no @warpgogol/werkstatt imports exist", async () => {
    writeFileSync(
  ...
  test("fails when @warpgogol/werkstatt is imported outside os/werkstatt/", async () => {
    writeFileSync(
    ...
    expect(result.data?.violations[0]!.specifier).toBe("@warpgogol/werkstatt/kernel");
    expect(result.exitCode).toBe(1);
  });

  test("passes when @warpgogol/werkstatt is imported inside os/werkstatt/", async () => {
    writeFileSync(
  ...
  test("reports multiple violations from different files", async () => {
    writeFileSync(
      ...
      `import { foo } from "@warpgogol/werkstatt";\n`,
    );
    writeFileSync(
      join(workspace, "packages", "forge", "os", "rfc", "bad2.ts"),
      `import { bar } from "@warpgogol/werkstatt/release";\n`,
    );
